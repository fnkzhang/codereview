...
export default function ProjectExport( props ) {
  ...
  const handleExportProject = async () => {
    ...
    let result = await getDeletedDocuments(project_id, gitRepo, repoBranch)
    if(!result.success){
      setWorking(false)
      setIsError(true)
      return
    }
    let deletedDocuments = result.body

    result = await getProjectDocuments(project_id)
    const documents = result
    
    let snapshots = []
    documents.forEach(async (element) => {
      result = await getAllSnapshotsFromDocument(project_id, element.doc_id)
      snapshots.push(result.body.pop().snapshot_id)
    });

    result = await pushToExistingBranch(project_id, gitRepo, repoBranch, deletedDocuments, snapshots, "Automatic commit generated by codereview web-app.")
    if (result.success){
